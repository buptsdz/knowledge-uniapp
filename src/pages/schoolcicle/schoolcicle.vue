			...
			<view class="rect" @tap.stop v-if="state==1">
				...
				<view class="choose-school">
					...
					<view class="input-section">
						<uni-combox class="choose-school-input" v-model="selectedSchoolName"
							:candidates="schoolCandidates.map(school => school.name)"
	...
	export default {
		...
		methods: {
			...
			fetchSchoolDetails(newVal) {
				...
				//发送请求
				this.$service.get("user-service/api/school")
					.then(response => {
						// 处理成功响应
						console.log("请求学校响应信息：", response);
						if (response.data.isSuccess == 1) {
							// 映射数据到包含name和id的对象
							this.schoolCandidates = response.data.data.data.map(school => ({
								name: school.name,
								id: school.id
							}));
							console.log("待选学校：", this.schoolCandidates);
							//找到当前学校对应的id
							const selectedSchool = this.schoolCandidates.find(school => school.name === newVal);
							if (selectedSchool) {
							...
							// 处理错误情况或者显示提示信息
							console.error('获取学校信息失败:', response.data.message);
						}
					})
					.catch(error => {
						// 处理错误
...
<style lang="scss">
	...
	.rect {
		...
		.quit::before {
			...
			background-image: radial-gradient(circle, rgb(85, 150, 254) 20%, transparent 20%),
				...
				radial-gradient(circle, rgb(85, 150, 254) 20%, transparent 20%),
				radial-gradient(circle, rgb(85, 150, 254) 20%, transparent 20%);
			background-position: 5% 30%, -5% 30%, 7% 10%, 23% -5%, 37% -10%, 58% -23%, 80% -20%, 100% -2%, -5% 80%,
				100% 55%, 10% 90%, 23% 110%, 42% 120%, 70% 110%, 78% 100%, 100% 80%;
			background-size: 0% 0%;
		...
		.quit:active::before {
			...
			background-position: 18% 40%, 20% 31%, 30% 22%, 40% 18%, 50% 16%, 57% 15%, 65% 18%, 80% 32%, 15% 60%,
				83% 60%, 18% 70%, 25% 73%, 41% 80%, 50% 84%, 64% 78%, 80% 71%;
		}
